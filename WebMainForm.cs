...
internal sealed class WebMainForm : Form
{
    ...
    private void State(bool replaceDocument = true)
    {
        ...
        var json = JsonSerializer.SerializeToNode(_document, MacroStorage.Options)!;
        for (var i = 0; i < _document.Nodes.Count; i++)
            json["Nodes"]![i]!["TargetId"] = _document.Nodes[i].SessionTarget?.Handle.ToInt64().ToString();
    ...
    private void SetDocument(JsonElement json)
    {
        ...
        if (doc.Version != 2) throw new InvalidDataException("В редакторе нужна Version: 2. Старые файлы открывайте через импорт.");
        MacroStorage.Validate(doc); var raw = json.GetProperty("Nodes");
        for (var i = 0; i < doc.Nodes.Count; i++)
        {
            var node = doc.Nodes[i];
            if (raw[i].TryGetProperty("TargetId", out var id) && id.ValueKind == JsonValueKind.String &&
                _windows.TryGetValue(id.GetString()!, out var target) && target.ProcessName == node.ProcessName && target.Title == node.WindowTitle)
                node.SessionTarget = target;
            else node.SessionTarget = _document.Nodes.FirstOrDefault(n => n.ProcessName == node.ProcessName && n.WindowTitle == node.WindowTitle)?.SessionTarget;
        }
        _document = doc;
    }
            ...
            try
            {
                ...
                switch (command)
                {
                    ...
                    case "open":
                        ...
                        { _document = MacroStorage.Read(path); _document.Version = 2; _file = path; _dirty = false; State(); }
                        break;
                    case "import":
                        ...
                        try
                        {
                            ...
                            { _document = MacroStorage.Read(picker.FileName); _document.Version = 2; _file = null; _dirty = true; State(); }
                        }
                        finally { _dialog = false; }
                        break;
                    case "export":
