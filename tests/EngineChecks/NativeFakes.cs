...
internal static class Native
{
    ...
    public static readonly List<string> WindowEvents = [];
    public static readonly Dictionary<string, WindowTarget> Targets = [];
    public static readonly HashSet<IntPtr> Closed = [];
    public static WindowTarget Active = new("active", (IntPtr)1);
    public static WindowTarget Target(string name)
    {
        if (!Targets.TryGetValue(name, out var target)) Targets[name] = target = new(name, (IntPtr)(Targets.Count + 1));
        return target;
    }
    public static void Emit(string text) { Events.Add(text); WindowEvents.Add(Active.Name + ":" + text); OnEvent?.Invoke(text); }
    public static void Reset() { Events.Clear(); WindowEvents.Clear(); Targets.Clear(); Closed.Clear(); Active = Target("active"); MouseTimes.Clear(); Focus = true; CursorInside = true; OnEvent = null; InputSender.SystemDoubleClickMs = 30; }
}
internal static class WindowTargeting
{
    public static WindowTarget GetActiveApplication()
    { EnsureForeground(Native.Active); return Native.Active; }
    public static WindowTarget Resolve(MacroNode node) => node.ProcessName == "missing"
        ? throw new TargetWindowException("missing") : node.SessionTarget ?? Native.Target(node.Name);
    public static Task ActivateAsync(WindowTarget target, CancellationToken token)
    {
        token.ThrowIfCancellationRequested();
        if (Native.Closed.Contains(target.Handle)) throw new TargetWindowException("closed");
        Native.Active = target; Native.Emit("window:" + target.Name); EnsureForeground(target); return Task.CompletedTask;
    }
    public static void EnsureForeground(WindowTarget target)
    { if (!Native.Focus || Native.Active.Handle != target.Handle || Native.Closed.Contains(target.Handle)) throw new TargetWindowException("focus lost or closed"); }
...
internal static class InputSender
{
    ...
    {
        "Ctrl" => (ushort)17, "C" => (ushort)67, "A" => (ushort)65, "Enter" => (ushort)13,
        "1" => (ushort)49, "2" => (ushort)50, "3" => (ushort)51, "4" => (ushort)52, "5" => (ushort)53,
